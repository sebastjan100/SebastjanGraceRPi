...
GPIO.setup(rdeca, GPIO.OUT)  # Set GPIO pin 12 to output mode.
pwm = GPIO.PWM(rdeca, 100)   # Initialize PWM on pwmPin 100Hz frequency
...
try:
  while True:                      # Loop until Ctl C is pressed to stop.
    for dc in range(0, 101, 5):    # Loop 0 to 100 stepping dc by 5 each loop
      ...
      time.sleep(0.05)             # 50 ms  wait .05 seconds at current LED brightness
      print(dc)
